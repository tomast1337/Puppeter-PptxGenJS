...
import { normalizeFill, normalizeColor } from "./style";
import { normalizeText, type TextInput } from "./text";

const INHERITED_CELL_OPTIONS = [
    "align", "bold", "border", "color", "fill", "fontFace", "fontSize",
    "margin", "textDirection", "underline", "valign",
] as const;
...
function tableWidthAndColumns(
    ...
    const repeatedWidth = typeof options.colW === "number"
        ? options.colW
        : widths?.length === 1 && count > 1 ? widths[0] : undefined;
    ...
    const width = options.w === undefined
        ? defaultWidth
        : convertToPixels(options.w, inchesToPixels(pageSize.width));
    return { width, columns: Array(count).fill(width / count) };
}

function rowHeights(rowCount: number, options: PptxGenJS.TableProps, pageSize: PageSize): Array<number | undefined> {
    const heightFromTable = options.h === undefined
        ? undefined
        : convertToPixels(options.h, inchesToPixels(pageSize.height)) / Math.max(1, rowCount);
...
export function normalizeTable(
    ...
    const explicitHeight = options.h === undefined
        ? heights.every(height => height !== undefined) ? heights.reduce<number>((sum, height) => sum + height!, 0) : undefined
    ...
    return {
        ...
        rows: tableRows.map((row, rowIndex) => ({
            ...
            cells: row.map(rawCell => {
                const cell = typeof rawCell === "object" && rawCell !== null
                    ? rawCell
                    : { text: String(rawCell) } as PptxGenJS.TableCell;
