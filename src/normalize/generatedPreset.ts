...
type PresetDefinition = {
    ...
    readonly paths: readonly {
        readonly w?: string; readonly h?: string; readonly fill?: string; readonly stroke?: string;
...
function variables(width: number, height: number): Record<string, number> {
    const values: Record<string, number> = {
        w: width, h: height, l: 0, t: 0, r: width, b: height,
        hc: width / 2, vc: height / 2, ss: Math.min(width, height), ls: Math.max(width, height),
        cd2: 180 * ANGLE, cd4: 90 * ANGLE, cd8: 45 * ANGLE, cd16: 22.5 * ANGLE,
        "3cd4": 270 * ANGLE, "3cd8": 135 * ANGLE, "5cd8": 225 * ANGLE, "7cd8": 315 * ANGLE,
    };
...
function evaluate(formula: string, values: Record<string, number>): number {
    ...
    switch (operator) {
        case "val": return x;
        case "pin": return Math.max(x, Math.min(y, z));
        case "*/": return z === 0 ? 0 : x * y / z;
        case "+-": return x + y - z;
        case "+/": return (x + y) / z;
        case "?:": return x > 0 ? y : z;
        case "abs": return Math.abs(x);
        case "at2": return Math.atan2(y, x) * 180 / Math.PI * ANGLE;
        case "cat2": return x * Math.cos(Math.atan2(z, y));
        case "cos": return x * Math.cos(angleRadians(y));
        case "max": return Math.max(x, y);
        case "min": return Math.min(x, y);
        case "mod": return Math.hypot(x, y, z);
        case "sat2": return x * Math.sin(Math.atan2(z, y));
        case "sin": return x * Math.sin(angleRadians(y));
        case "sqrt": return Math.sqrt(Math.max(0, x));
        case "tan": return x * Math.tan(angleRadians(y));
        default: throw new Error(`Unsupported DrawingML formula: ${operator}`);
    }
}
...
function renderPath(
    path: PresetDefinition["paths"][number],
    ...
    for (const command of path.commands) {
        ...
        if (kind === "M" || kind === "L") {
            ...
            commands.push(arc.data); current = arc.end;
        } else if (kind === "Q") {
...
export function generatedPresetGeometry(name: PresetName, width: number, height: number): Extract<NormalizedShape["geometry"], { kind: "path" }> {
    ...
    const faces = rendered.filter(({ path }) => path.fill !== "none").map(({ path, data }) => ({
        data,
        ...(["darken", "darkenLess", "lighten", "lightenLess"].includes(path.fill ?? "")
            ? { fillModifier: path.fill as "darken" | "darkenLess" | "lighten" | "lightenLess" } : {}),
    }));
    const outlineData = rendered.filter(({ path }) => path.stroke !== "false").map(({ data }) => data).join(" ");
