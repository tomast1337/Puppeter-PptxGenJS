...
interface Bar { category: number; series: number; base: number; value: number }
...
function shade(hex: string, amount: number): string {
    ...
    const adjusted = channels.map(channel => amount >= 0
...
export function renderBar3DSvg(width: number, height: number, chart: NormalizedChart): string {
    ...
    const grid = Array.from({ length: tickCount + 1 }, (_, index) => {
        ...
        return `<line x1="${n(left.x)}" y1="${n(left.y)}" x2="${n(right.x)}" y2="${n(right.y)}" stroke="#${chart.gridLineColor.replace(/^#/, "")}" stroke-width="${n(pointsToPixels(chart.gridLineWidth))}"/>`
    ...
    const barSvg = bars.map(bar => {
        ...
        if (chart.bar3DShape === "pyramid" || chart.bar3DShape === "pyramidToMax") {
            ...
            return `<g class="${shapeClass}" data-series="${bar.series}" data-category="${bar.category}" opacity="${opacity}">`
                + polygon("bar3d-front", [bfl, bfr, apex], frontColor)
                + polygon("bar3d-side", [bfr, bbr, apex], sideColor)
                + polygon("bar3d-back", [bbr, bbl, apex], topColor) + `</g>`;
        }
        ...
        return `<g class="${shapeClass}" data-series="${bar.series}" data-category="${bar.category}" opacity="${opacity}">`
            ...
            + polygon("bar3d-top", [tfl, tfr, tbr, tbl], topColor) + `</g>`;
    }).join("");
    ...
    const seriesLabels = chart.series.map((series, index) => {
    ...
    const legend = chart.showLegend ? chart.series.map((series, index) => {
        ...
        return `<rect x="${n(x - 34)}" y="${n(y - 8)}" width="12" height="8" fill="${color}"/>`
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${n(width)} ${n(height)}" data-chart-type="bar3D" data-bar3d-shape="${chart.bar3DShape}">`
        + `<g font-family="${escapeXml(chart.fontFace)}" font-size="${n(pointsToPixels(chart.fontSize))}" fill="#000000">`
        + gradients + title + wall + floor + frame + grid + barSvg + categoryLabels + seriesLabels + legend + `</g></svg>`;
}
