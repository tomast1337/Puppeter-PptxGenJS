...
const rows = (count: number): PptxGenJS.TableRow[] => Array.from({ length: count }, (_, index) => [
    { text: `Row ${index + 1}` },
    { text: `Value ${index + 1}` },
]);
...
describe("table auto-pagination", () => {
    test("allocates rows using PptxGenJS line-height and continuation heuristics", () => {
        ...
        const heavierLines = paginateTableRows(rows(30), {
            autoPage: true, y: 1, w: 8, autoPageLineWeight: 1,
        }, PAGE);
    ...
    test("repeats the requested headers and honors the continuation start", () => {
        const input = rows(30);
        const pages = paginateTableRows(input, {
            autoPage: true,
            y: 1,
            w: 8,
    ...
    test("splits long rows without losing text and applies character weight", () => {
        ...
        const input = [[{ text }]];
        const narrow = paginateTableRows(input, {
            autoPage: true, y: 1, w: 2, autoPageCharWeight: -1,
        }, PAGE);
        const wide = paginateTableRows(input, {
            autoPage: true, y: 1, w: 2, autoPageCharWeight: 1,
        }, PAGE);
        expect(narrow.length).toBeGreaterThan(wide.length);

        const reconstructed = narrow.flatMap(page => page.rows).flatMap(row => {
