...
describe("table normalization", () => {
    test("normalizes explicit columns, rows, spans, and per-side borders", () => {
        ...
        expect(table.rows[0]!.cells[0]!.borders.map(border => [border.visible, border.style, border.color, border.width]))
            .toEqual([
    ...
    test("inherits table typography, fill, margins, and direction into cells", () => {
        ...
        expect(cell.text.paragraphs[0]!.runs.map(run => [run.text, run.bold, run.italic, run.color]))
            .toEqual([
    ...
    test("matches default, scalar, and mismatched column width behavior", () => {
        ...
        expect(normalizeTable([rows[0]!, rows[0]!], { h: 2, rowH: [0.5] }, PAGE).rows.map(row => row.height))
            .toEqual([48, 96]);
    });
...
describe("table rendering", () => {
    test("renders colgroups, merged cells, rich runs, and inherited styling", () => {
        ...
        slide.addTable([[
            {
                text: [{ text: "One", options: { bold: true } }, { text: " two", options: { italic: true } }],
                options: { colspan: 2, rowspan: 2, fill: { color: "D9EAF7" } },
            },
            { text: "Three" },
        ], [{ text: "Four" }]], {
            x: 1, y: 1, colW: [1, 1, 2], rowH: 0.5,
